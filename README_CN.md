...
- SenseVoice 语音模型 — coli 首次使用时下载，约 230 MB

如果依赖缺失，TypeNo 会在录制前弹出应用内设置向导。检测到 Homebrew 时，应用可以自动安装 `ffmpeg`；npm 和 ffmpeg 就绪后，也可以自动安装 `coli`；随后会提前下载首次使用所需的 SenseVoice 模型，避免第一次录音时卡在模型下载。如果缺少 Node.js 或 Homebrew，向导会提供安装链接和一键复制的命令。
